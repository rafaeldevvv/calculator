...
export function renderHistoryCount(count) {
    return `<p id="history-count" class="fw-600 mt-3 mb-1">${count} calculation${count === 1 ? "" : "s"} stored:</p>`;
}
export function renderHistoryControls(numOfEntries, numOfShownEntries, least) {
    const disableSeeMore = numOfEntries <= numOfShownEntries, disableSeeLess = numOfShownEntries === least;
    return `
